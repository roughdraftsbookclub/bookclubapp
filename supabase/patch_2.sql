-- Fix: to_char(..., 'Month YYYY') blank-pads the month name to 9 characters
-- (Postgres fixed-width behavior), so the first real publish wrote
-- "August    2026" (extra internal spaces) instead of "August 2026". trim()
-- only strips the outer edges, not that internal padding — the FM (fill
-- mode) prefix is what actually removes it.
...
-- One-off fix for the bad value the first real publish already wrote.
update books set date_read = 'August 2026' where id = 'bk18' and date_read = 'August    2026';
